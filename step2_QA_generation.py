...
# Load CSV file from Step1 (Data extraction)
...
You are an expert in clinical reasoning. Your task is to generate single-entity, medically factual and causal QA pairs based on a discharge summary.

Instructions:
- Each QA pair should involve at least **one distinct medical entity** (e.g., a diagnosis, a lab result, or a medication).
- The **question** should ask about the reason or cause for the entity or an outcome that results from its presence.
- The **answer** should explain the **causal relationship** connecting the entity and the outcome.
- Then, generate a **counterfactual version** of the question and answer, explaining what would happen if the entity were not present.
...
Output format:
Factual QA:
Q: <Single-entity causal question>
A: <Answer explaining how the entity caused or contributed to outcome>

Counterfactual QA:
Q: <What if... question removing the entity>
A: <Answer explaining how absence of the entity changes outcome>
"""
...
single_or_multi_entity = sys.argv[1] if sys.argv[1] else "single"  # "single" or "multi"
PROMPT_TEMPLATE = PROMPT_TEMPLATE_SINGLE if single_or_multi_entity == "single" else PROMPT_TEMPLATE_MULTI
NUM_QA_PAIRS = int(sys.argv[2]) if len(sys.argv) > 2 else 2   

print(f"Generating #{NUM_QA_PAIRS} {single_or_multi_entity}-entity QA pairs (both factual and counterfactual)")

# Run generation
df = df.head(NUM_QA_PAIRS) 
raw_results = []

for idx, row in tqdm(df.iterrows(), total=len(df)):
    print(f"Generating {idx+1} QA pair")
...
###################################################################################################################
print(f"QA pairs are generated and now processing them.....")
...
# Save Final Output
out_path = f"data_final/step2_{single_or_multi_entity}_{NUM_QA_PAIRS}.jsonl"
